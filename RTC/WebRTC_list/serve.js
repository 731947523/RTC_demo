...
io.sockets.on('connection', (socket) => {
  ...
  socket.on('message', (message) => { //收到message时，进行广播
    io.in(mYroom).emit('message', message) //在真实的应用中，应该只在房间内广播
  })
  ...
  socket.on('create or join', (room) => { //收到 “create or join” 消息
    ...
    console.log('create or join.....', io.sockets.adapter.rooms[room])
    var numClients = clientsInRoom ? Object.keys(clientsInRoom.sockets).length : 0 //房间里的人数
    console.log('create or join.....1', numClients)
    mYroom = room
    // console.log('create or join.....2', clientsInRoom)
    // console.log('create or join.....3', numClients)
